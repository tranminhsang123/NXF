...
<head>
    ...
    <title>{{ $courseData['title'] }} - Speed Master N5</title>
            ...
            <div class="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
                ...
                <div class="mt-5 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        ...
                        <h1 class="mt-1 text-3xl font-black tracking-tight text-slate-950">Speed Master N5</h1>
                        <p class="mt-2 max-w-2xl text-sm leading-6 text-slate-600">Học nhanh và hiệu quả với giáo trình Speed Master N5.</p>
                    </div>
                    <div class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
                        <p class="text-xs font-bold uppercase tracking-wide text-slate-500">Số bài</p>
                        <p class="mt-1 text-2xl font-black text-slate-950">{{ count($lessons) }}</p>
                    </div>
                </div>
            </div>
        </section>
        ...
        <section class="py-6 sm:py-8">
            <div class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
                @foreach($lessons as $index => $lesson)
                    <a href="{{ route('course.speed-master.detail', ['level' => $level, 'bai' => $lesson['bai']]) }}" 
                ...
                @if(empty($lessons))
                    <div class="rounded-xl border border-slate-200 bg-white p-8 text-center">
                        <p class="text-slate-500">Chưa có bài Speed Master N5</p>
                    </div>
                @endif
            </div>
        </section>
