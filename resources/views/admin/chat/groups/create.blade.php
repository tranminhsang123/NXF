...
@section('content')
    <div class="mb-6">
        <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <h1 class="text-2xl md:text-3xl font-bold text-gray-900">Tạo nhóm chat</h1>
            <a href="{{ route('admin.chat.groups.index') }}"
               class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg font-semibold text-center">
                ← Quay lại
            </a>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <form method="POST" action="{{ route('admin.chat.groups.store') }}">
            @csrf
            ...
            <div class="mb-5">
                ...
                <input id="name" name="name" type="text" value="{{ old('name') }}"
                       ...
                       placeholder="Ví dụ: N5 - Nhóm học tập">
                @error('name')
            ...
            <div class="mb-4">
                <div class="flex items-center justify-between">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Chọn user để tham gia *</label>
                    <div class="text-xs text-gray-500">
                        Tick để thêm thành viên vào nhóm chat
                    </div>
                </div>

                @php $oldIds = old('user_ids', []); @endphp
